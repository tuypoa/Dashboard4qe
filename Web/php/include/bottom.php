...
<tr>
	...
	$query = "
		SELECT m.codigo, m.nome, p.conteudo,
			...
			((MAX(qie.cputime) - MIN(qie.cputime))/3600) as horas
		FROM psaux p 
	...
	<tr>
		...
		<pre><?php
			foreach ($rsBusca as $obj){
				...
				echo " ".$obj["datahora"]." <a href='maquina.php?mid=".$obj["codigo"]."' style='font-weight:bold;text-decoration:none;' ".($maquina!=null && $obj["codigo"]==$maquina["codigo"]?" class='fblue'":"").">".$obj["nome"]."</a>$ ".$obj["conteudo"];
				if($obj["ciclos"]!=null) { 
					echo " <span class='".($op["concluido"]?"fgreen":"fred")."'>(".$obj["ciclos"]." steps, ".($obj["horas"]>72?number_format($obj["horas"]/24,0)."d".number_format($obj["horas"]%24,0)."h":$obj["horas"]."h").")</span>"; 
				}
				echo "\n";
